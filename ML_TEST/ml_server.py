...
def extract_features(readings):
    ...
    mq3 = np.array([float(r["mq3"]) for r in readings], dtype=float)
    mq135 = np.array([float(r["mq135"]) for r in readings], dtype=float)
    temperature = np.array([float(r["temperature"]) for r in readings], dtype=float)
    humidity = np.array([float(r["humidity"]) for r in readings], dtype=float)

    vmq3_raw = np.array([convert_sensor(x, "MQ3") for x in mq3], dtype=float)
    vmq135_raw = np.array([convert_sensor(x, "MQ135") for x in mq135], dtype=float)

    vmq3 = np.array([
        compensate_sensor(vmq3_raw[i], temperature[i], humidity[i], "MQ3")
        for i in range(30)
    ], dtype=float)

    vmq135 = np.array([
        compensate_sensor(vmq135_raw[i], temperature[i], humidity[i], "MQ135")
        for i in range(30)
    ], dtype=float)

    VMQ3 = float(np.mean(vmq3[-5:]))
    VMQ135 = float(np.mean(vmq135[-5:]))
    temperature_mean = float(np.mean(temperature[-5:]))
    humidity_mean = float(np.mean(humidity[-5:]))
...
def detect_sensor_region(features):
    ...
    vmq3 = float(features[0])
    vmq135 = float(features[1])
...
def predict(features, device_id):
    ...
    prediction = label_classes[prediction_index]
    confidence = float(adjusted_probabilities[prediction])
...
def predict_endpoint():
    ...
    try:
        ...
        response = {
            ...
            "features": {
                "VMQ3": round(float(features[0]), 5),
                "VMQ135": round(float(features[1]), 5),
                "temperature": round(float(features[2]), 2),
                "humidity": round(float(features[3]), 2),
            }
        }
