    ...
    <div className={`min-h-full ${bg}`}>
      <div className="p-4 md:p-6 space-y-4 pb-20 md:pb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
          {[
            { l: "TOTAL ENTRIES",  v: logs.length,                                   c: "#F4B942" },
            { l: "ALERTS",         v: logs.filter(l => l.level === "ALERT").length,   c: "#E05252" },
            { l: "WARNINGS",       v: logs.filter(l => l.level === "WARNING").length, c: "#F4B942" },
            { l: "SUCCESSFUL OPS", v: logs.filter(l => l.level === "SUCCESS").length, c: "#3FB950" },
          ].map(s => (
              ...
              {["ALL", "DEVICES", "ALERTS"].map(fi => (
                <button key={fi} onClick={() => setF(fi)}
                  className={`font-mono text-[8.5px] tracking-widest px-2 py-1 border transition-all
                    ${f === fi ? "bg-brass text-obsidian border-brass" : `${bdr} ${muted} hover:border-brass`}`}>
                  {fi}
                </button>
              ))}
            </div>
