...
import { DEMO_DEVICES, DEMO_INCIDENTS, DEMO_LOGS, truncateConfidencePercent, type Device, type Incident, type LogEntry } from "./data";
...
export default function App() {
  ...
  const [devId, setDevId]         = useState("DART-032");
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const [apiDevices, apiSummary, apiIncidents, apiLogs] = await Promise.all([
          getDevices(), getDashboardSummary(), getIncidents(), getLogs(),
        ]);
        if (!active) return;
        const nextDevices = apiDevices.length > 0 ? apiDevices : DEMO_DEVICES;
        const nextIncidents = apiIncidents.length > 0 ? apiIncidents : DEMO_INCIDENTS;
        const nextLogs = apiLogs.length > 0 ? apiLogs : DEMO_LOGS;
        const nextSummary = apiDevices.length > 0
          ? apiSummary
          : { connectedDevices: DEMO_DEVICES.length, online: DEMO_DEVICES.filter(device => device.status === "ONLINE").length, offline: DEMO_DEVICES.filter(device => device.status === "OFFLINE").length, activeAlerts: DEMO_INCIDENTS.length };
        ...
        if (seenIncidentIds.current) {
          nextIncidents
            .filter(incident => !seenIncidentIds.current?.has(incident.id) && incident.status !== "RESOLVED")
            .forEach(incident => {
              const isWarning = incident.type === "ALCOHOL";
              addToast(
                `${isWarning ? "ALCOHOL SCREENING" : "ANOMALY DETECTED"} — ${incident.device} / ${incident.location} — ${incident.type} ${truncateConfidencePercent(incident.confidence)}%`,
        ...
        setSummary({ connectedDevices: DEMO_DEVICES.length, online: DEMO_DEVICES.filter(device => device.status === "ONLINE").length, offline: DEMO_DEVICES.filter(device => device.status === "OFFLINE").length, activeAlerts: DEMO_INCIDENTS.length });
  ...
  const activeThreatIncident = incidents.find(incident => incident.status !== "RESOLVED");
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-4 md:px-6 py-2.5 border-b bg-charcoal border-warm-grey/10">
          <div className="flex items-center gap-3 md:pl-0 pl-12">
            <div className="font-mono text-[9.5px] tracking-[.18em] uppercase text-warm-grey">
              <span className="text-brass">DART</span> // POLICE STATION CONTROL CENTER // AREA COMMAND
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {screen==="anomalies"&&(
            <AnomalyAlert theme="dark"
              ...
              onAcknowledge={async id=>{ await handleAcknowledge(id); addToast("Issue recognised — AREA_POLICE_001","success","SAFE"); }}
