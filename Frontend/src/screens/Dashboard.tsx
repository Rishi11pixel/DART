...
const RESULT_COLOR: Record<string,string> = { "SAFE":"#3FB950","CAUTION":"#F4B942","ALCOHOL":"#F4B942","NARCOTIC PROXY":"#E05252" };
...
function MetricCard({ label, value, desc, badge, badgeColor, trend, dark }:{
  ...
  return (
    <div className={`relative overflow-hidden p-4 panel ${dark?"bg-gunmetal":"bg-white panel-light"}`}>
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-brass/45"/>
      ...
      {trend && (
        <div className="absolute top-3 right-3">
          <span className={`font-mono text-[9px] ${trend==="up"?"text-brass":trend==="down"?"text-safe":"text-warm-grey"}`}>
            {trend==="up"?"↑":trend==="down"?"↓":"→"}
          </span>
        </div>
      )}
  ...
  return (
    <div className={`relative w-full h-72 md:h-[340px] overflow-hidden ${dark?"bg-[#0A0B0D]":"bg-[#E0DBD0]"} grid-bg`}>
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 640 380" preserveAspectRatio="xMidYMid meet">
        ...
        <text x="310" y="68" textAnchor="middle" fontSize="8" fill="#F4B942" fillOpacity=".6" fontFamily="JetBrains Mono, monospace" letterSpacing="2">NEAR KENDRIYA VIDYALAYA</text>

        {/* Sector-10,11 Dividing Road */}
        ...
        <text x="310" y="168" textAnchor="middle" fontSize="8" fill="#F4B942" fillOpacity=".6" fontFamily="JetBrains Mono, monospace" letterSpacing="1">SECTOR-10,11 DIVIDING ROAD</text>
        ...
        <text x="480" y="283" textAnchor="middle" fontSize="7" fill="#F4B942" fillOpacity=".5" fontFamily="JetBrains Mono, monospace" letterSpacing="1">NEAR MEDICAL COLLEGES</text>
        ...
        {/* Police station response area */}
        <rect x="200" y="270" width="165" height="70" rx="0" fill={dark?"#14151720":"#D4CFC520"} stroke="#C49A4A" strokeWidth=".6" strokeOpacity=".18"/>
        <text x="283" y="290" textAnchor="middle" fontSize="7" fill="#F4B942" fillOpacity=".5" fontFamily="JetBrains Mono, monospace" letterSpacing="1">POLICE STATION RESPONSE</text>

        {/* Compass / scale */}
        <text x="610" y="360" textAnchor="end" fontSize="6.5" fill="#F4B942" fillOpacity=".3" fontFamily="JetBrains Mono, monospace">AREA // SIM</text>
        ...
        <rect x="49" y="38" width="112" height="54" fill={dark ? "#15171A" : "#EEF0F2"} stroke="#F4B942" strokeOpacity=".5" />
        <text x="105" y="62" textAnchor="middle" fontSize="7" fill="#F4B942" fontFamily="JetBrains Mono, monospace">NEAR KENDRIYA</text>
        <text x="105" y="73" textAnchor="middle" fontSize="7" fill="#F4B942" fontFamily="JetBrains Mono, monospace">VIDYALAYA</text>
        ...
        <rect x="414" y="242" width="164" height="68" fill={dark ? "#15171A" : "#EEF0F2"} stroke="#F4B942" strokeOpacity=".5" />
        <text x="496" y="270" textAnchor="middle" fontSize="7" fill="#F4B942" fontFamily="JetBrains Mono, monospace">NEAR MEDICAL</text>
        <text x="496" y="281" textAnchor="middle" fontSize="7" fill="#F4B942" fontFamily="JetBrains Mono, monospace">COLLEGES</text>
        ...
        <path d="M45 343h70" stroke="#F4B942" strokeWidth="2" />
        <text x="45" y="355" fontSize="6" fill="#F4B942" fontFamily="JetBrains Mono, monospace">100 M APPROX.</text>
      </svg>
      ...
      <div className={`absolute bottom-3 right-3 p-2.5 ${dark?"bg-charcoal/90":"bg-white/90"} border ${dark?"border-warm-grey/10":"border-obsidian/10"}`}>
          {[['ONLINE',"#3FB950"],["DRUG ALERT","#E05252"],["ALCOHOL","#F4B942"]].map(([l,c])=>(
      ...
      <div className="relative z-10 p-4 md:p-6 space-y-5 pb-20 md:pb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
          <div>
            ...
            <div className={`font-mono text-[9.5px] tracking-widest uppercase mt-1 ${muted}`}>Real-time overview of connected DART devices and area police station security systems.</div>
          </div>
          <div className="flex flex-col items-end gap-2 flex-shrink-0">
            ...
            <div className={`flex items-center gap-2 px-3 py-2 border ${bdr} ${cBg}`}>
              <span className={`font-mono text-[9.5px] tracking-widest ${muted}`}>
                <span className="text-brass">STATION //</span> AREA POLICE COMMAND
              </span>
            </div>
          ...
          <div className={`px-4 py-3 flex items-center justify-between border-b ${bdr}`}>
            <div>
              <div className={`font-heading text-[12px] tracking-[.14em] font-semibold uppercase ${text}`}>CONNECTED SENTRY DEVICES</div>
                <div className={`font-mono text-[9px] tracking-widest ${muted} mt-0.5`}>Devices operating near Kendriya Vidyalaya, Sector-10,11 Dividing Road and Medical Colleges</div>
            </div>
        ...
        <div className={`flex items-center justify-between pt-4 border-t ${bdr}`}>
          <div className={`font-mono text-[8.5px] tracking-widest ${muted}`}>SENTRY OS v1.0.0 // DATA SHOWN IS SIMULATED — DEMO MODE</div>
                <div className={`font-mono text-[8.5px] tracking-widest ${muted}`}>SESSION: AREA_POLICE_001 // STATION-SEC</div>
        </div>
      </div>
    </div>
  );
